...
do
    ...
    local Files = {
        Lua = {
            Path = SCRIPT_PATH,
            Name = "Evelynn.lua",
            Url = "https://raw.githubusercontent.com/miragessee/GoSEvelynn/master/Evelynn.lua"
        },
        Version = {
            Path = SCRIPT_PATH,
            Name = "miragesevelynn.version",
            Url = "https://raw.githubusercontent.com/miragessee/GoSEvelynn/master/miragesevelynn.version"
        }
    }
...
function QDmg()
    if myHero:GetSpellData(_Q).level == 0 then
        local Dmg1 = (({25, 30, 35, 40, 45})[1] + 0.30 * myHero.ap)
        return Dmg1
    else
        local Dmg1 = (({25, 30, 35, 40, 45})[myHero:GetSpellData(_Q).level] + 0.30 * myHero.ap)
        return Dmg1
    end
end
...
function EDmg(target)
    if myHero:GetSpellData(_E).level == 0 then
        local Bonus = myHero.ap / 100 * 1.5 + 0.03 * target.maxHealth
        local EDmg = ({55, 70, 85, 100, 115})[1] + Bonus
        return EDmg
    else
        local Bonus = myHero.ap / 100 * 1.5 + 0.03 * target.maxHealth
        local EDmg = ({55, 70, 85, 100, 115})[myHero:GetSpellData(_E).level] + Bonus
        return EDmg
    end
end
...
function RDmg()
    if myHero:GetSpellData(_R).level == 0 then
        local Dmg1 = (({150, 275, 400})[1] + 0.75 * myHero.ap)
        return Dmg1
    else
        local Dmg1 = (({150, 275, 400})[myHero:GetSpellData(_R).level] + 0.75 * myHero.ap)
        return Dmg1
    end
end
...
function RDmg2()
    if myHero:GetSpellData(_R).level == 0 then
        local Dmg1 = (({300, 550, 800})[1] + 1.5 * myHero.ap)
        return Dmg1
    else
        local Dmg1 = (({300, 550, 800})[myHero:GetSpellData(_R).level] + 1.5 * myHero.ap)
        return Dmg1
    end
end
...
function TestBuff(unit)
    for i = 0, unit.buffCount do
        ...
        if buff and buff.count > 0 and buff.name == "EvelynnW" then
            print(buff.duration)
        end
    end
    print("No buff")
...
local HeroIcon = "https://www.mobafire.com/images/champion/square/evelynn.png"
local IgniteIcon = "http://pm1.narvii.com/5792/0ce6cda7883a814a1a1e93efa05184543982a1e4_hq.jpg"
local QIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/d/d7/Hate_Spike.png"
local WIcon = "https://vignette.wikia.nocookie.net/leagueoflegends/images/1/16/Allure.png"
...
function Evelynn:LoadMenu()
    ...
    self.EvelynnMenu = MenuElement({type = MENU, id = "Evelynn", name = "Mirage's Evelynn", leftIcon = HeroIcon})
    
    self.EvelynnMenu:MenuElement({id = "Harass", name = "Harass", type = MENU})
    self.EvelynnMenu.Harass:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseW", name = "Use W", value = false, leftIcon = WIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseE", name = "Use E", value = false, leftIcon = EIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseR", name = "Use R ", value = false, leftIcon = RIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseRl", name = "Use R least health enemy", value = false, leftIcon = RIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseRKS", name = "Use R is killable", value = true, leftIcon = RIcon})
    self.EvelynnMenu.Harass:MenuElement({id = "UseHP", name = "Use Hextech Probelt", value = true, leftIcon = HPIcon})
    
    self.EvelynnMenu:MenuElement({id = "Combo", name = "Combo", type = MENU})
    self.EvelynnMenu.Combo:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseW", name = "Use W Only charm", value = true, leftIcon = WIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseE", name = "Use E", value = true, leftIcon = EIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseR", name = "Use R ", value = false, leftIcon = RIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseRl", name = "Use R least health enemy", value = false, leftIcon = RIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseRKS", name = "Use R is killable", value = true, leftIcon = RIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseHP", name = "Use Hextech Probelt", value = true, leftIcon = HPIcon})
    self.EvelynnMenu.Combo:MenuElement({id = "UseSmite", name = "Use smite", value = true, leftIcon = SmiteIcon})
    
    self.EvelynnMenu:MenuElement({id = "KillSteal", name = "KillSteal", type = MENU})
    self.EvelynnMenu.KillSteal:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    ...
    self.EvelynnMenu.Clear:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
    self.EvelynnMenu.Clear:MenuElement({id = "UseE", name = "Use E", value = true, leftIcon = EIcon})
    
    self.EvelynnMenu:MenuElement({id = "AutoLevel", name = "AutoLevel", type = MENU})
    self.EvelynnMenu.AutoLevel:MenuElement({id = "AutoLevel", name = "Only Q->E->W", value = true})
    
    self.EvelynnMenu:MenuElement({id = "Escape", name = "Escape", type = MENU})
    self.EvelynnMenu.Escape:MenuElement({id = "UseHP", name = "Use Hextech Probelt", value = true, leftIcon = HPIcon})
    
    self.EvelynnMenu:MenuElement({id = "Drawings", name = "Drawings", type = MENU})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawQ", name = "Draw Q Range", value = true})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawW", name = "Draw W Range", value = true})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawE", name = "Draw E Range", value = true})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawR", name = "Draw R Range", value = true})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawAA", name = "Draw Killable AAs", value = false})
    self.EvelynnMenu.Drawings:MenuElement({id = "DrawJng", name = "Draw Jungler Info", value = true})
    
    self.EvelynnMenu:MenuElement({id = "blank", type = SPACE, name = ""})
    self.EvelynnMenu:MenuElement({id = "blank", type = SPACE, name = "Script Ver: " .. Version .. " - LoL Ver: " .. LVersion .. ""})
    self.EvelynnMenu:MenuElement({id = "blank", type = SPACE, name = "by " .. Author .. ""})
end
...
function Evelynn:Tick()
    ...
    if self.EvelynnMenu.AutoLevel.AutoLevel:Value() then
...
function Evelynn:CollisionX(myHeroPos, dangerousPos, unitPos, radius)
...
function Evelynn:CalculateEndPos(startPos, placementPos, unitPos, range)
    if range > 0 then
...
function Evelynn:ProcessSpell(units)
    for i = 1, #units do
        local unit = units[i]
    ...
    for key, v in pairs(IS) do
        ...
        if GetDistanceSqr(SpellHit, SpellPosition) <= GetDistanceSqr(dodge, SpellPosition) and GetDistance(SpellHit, v.sPos) - v.radius - myHero.boundingRadius <= GetDistance(v.sPos, v.ePos) then
            if GetDistanceSqr(myHero.pos, SpellHit) < (v.radius + myHero.boundingRadius) ^ 2 then
                if self.EvelynnMenu.Escape.UseHP:Value() then
...
function Evelynn:KillSteal()
    if self.EvelynnMenu.KillSteal.UseIgnite:Value() then
        for i, enemy in pairs(GetEnemyHeroes()) do
    ...
    if self.EvelynnMenu.KillSteal.UseHP:Value() then
        if GetItemSlot(myHero, 3152) > 0 then
            for i, enemy in pairs(GetEnemyHeroes()) do
    ...
    if self.EvelynnMenu.KillSteal.UseR:Value() then
        if IsReady(_R) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, EvelynnR.range) and (enemy.health / enemy.maxHealth <= 29 / 100) then
                    if enemy.health + enemy.shieldAP < RDmg2() then
                        LocalControlCastSpell(HK_R, enemy)
                    end
                elseif ValidTarget(enemy, EvelynnR.range) and enemy.health + enemy.shieldAP < RDmg() then
                    LocalControlCastSpell(HK_R, enemy)
                end
            end
        end
    end
    if self.EvelynnMenu.KillSteal.UseQ:Value() then
        if IsReady(_Q) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, EvelynnQ.range) and enemy.health + enemy.shieldAP < QDmg() then
                    LocalControlCastSpell(HK_Q, enemy)
                end
            end
        end
    end
    if self.EvelynnMenu.KillSteal.UseE:Value() then
        if IsReady(_E) then
            for i, enemy in pairs(GetEnemyHeroes()) do
                if ValidTarget(enemy, EvelynnE.range) and enemy.health + enemy.shieldAP < EDmg(enemy) then
...
function Evelynn:Harass()
    --local targetW = GOS:GetTarget(GetWRange(), "AP")
    --print(TestBuff(targetW))
    --print(GotBuff(targetW, "EvelynnW"))
    --print(GetWTarget())
    --if GetWTarget() then
    --    print("works")
    --end
    ...
    if not IsImmune(target) then
        if self.EvelynnMenu.Harass.UseW:Value() then
            if self.CollisionSpellName == "YasuoWMovingWall" then
                
                else
                if IsReady(_W) then
                    if ValidTarget(target, GetWRange()) then
                        LocalControlCastSpell(HK_W, target)
                    end
                end
            end
        end
    ...
    if self.EvelynnMenu.Harass.UseE:Value() then
        if IsReady(_E) and self.Collision == false then
            if ValidTarget(target, EvelynnE.range) then
    ...
    if self.EvelynnMenu.Harass.UseHP:Value() then
        if GetItemSlot(myHero, 3152) > 0 and ValidTarget(target, 850) then
            if myHero:GetSpellData(GetItemSlot(myHero, 3152)).currentCd == 0 then
                Control.CastSpell(Item_HK[GetItemSlot(myHero, 3152)], target)
            end
        end
    end
    if self.EvelynnMenu.Harass.UseR:Value() then
        if IsReady(_R) then
            if ValidTarget(target, EvelynnR.range) then
                LocalControlCastSpell(HK_R, target)
            end
        end
    end
    
    if self.EvelynnMenu.Harass.UseRKS:Value() then
        if IsReady(_R) then
            if ValidTarget(target, EvelynnR.range) and (target.health / target.maxHealth <= 29 / 100) then
                if target.health + target.shieldAP < RDmg2() then
                    LocalControlCastSpell(HK_R, target)
                end
            elseif ValidTarget(target, EvelynnR.range) and target.health + target.shieldAP < RDmg() then
                LocalControlCastSpell(HK_R, target)
            end
        end
    end
    ...
    if self.EvelynnMenu.Harass.UseRl:Value() then
        local minHealth = 50000
        local minHealthEnemy
        
        for i, enemy in pairs(GetEnemyHeroes()) do
            if ValidTarget(enemy, EvelynnR.range) and enemy.health < minHealth then
...
function Evelynn:CastQ(target, QcastPos)
    if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
        if GetDistance(myHero.pos, QcastPos) <= EvelynnQ.range then
...
function Evelynn:Combo()
    ...
    if self.EvelynnMenu.Combo.UseSmite:Value() then
    ...
    --print("works2")
    if not IsImmune(target) then
        if self.EvelynnMenu.Combo.UseW:Value() then
            if self.CollisionSpellName == "YasuoWMovingWall" then
    ...
    if IsReady(_W) or Game.CanUseSpell(_W) == 8 then
        local targetW = GetWTarget()
        
        if targetW then
            if not IsImmune(targetW) then
                if self.EvelynnMenu.Combo.UseE:Value() then
                    if IsReady(_E) and self.Collision == false then
                        if ValidTarget(targetW, EvelynnE.range) then
                            LocalControlCastSpell(HK_E, targetW)
                        end
                    end
                end
                ...
                if self.EvelynnMenu.Combo.UseHP:Value() then
                    if GetItemSlot(myHero, 3152) > 0 and ValidTarget(targetW, 850) then
                        if myHero:GetSpellData(GetItemSlot(myHero, 3152)).currentCd == 0 then
                            Control.CastSpell(Item_HK[GetItemSlot(myHero, 3152)], targetW)
                        end
                    end
                end
                if self.EvelynnMenu.Combo.UseR:Value() then
                    if IsReady(_R) then
                        if ValidTarget(targetW, EvelynnR.range) then
                            LocalControlCastSpell(HK_R, targetW)
                        end
                    end
                end
            end
    ...
    if self.EvelynnMenu.Combo.UseRKS:Value() then
        if IsReady(_R) then
            if ValidTarget(target, EvelynnR.range) and (target.health / target.maxHealth <= 29 / 100) then
                if target.health + target.shieldAP < RDmg2() then
                    LocalControlCastSpell(HK_R, target)
    ...
    if self.EvelynnMenu.Combo.UseRl:Value() then
        local minHealth = 50000
        local minHealthEnemy
...
function Evelynn:Clear()
    if self.EvelynnMenu.Clear.UseE:Value() then
        for i = 1, LocalGameMinionCount() do
            local minion = LocalGameMinion(i)
            if minion and minion.isEnemy then
                if IsReady(_E) then
                    if ValidTarget(minion, EvelynnE.range) then
    ...
    if self.EvelynnMenu.Clear.UseQ:Value() then
        for i = 1, LocalGameMinionCount() do
            local minion = LocalGameMinion(i)
            if minion and minion.isEnemy then
                if IsReady(_Q) then
                    if ValidTarget(minion, EvelynnQ.range) then
...
function Evelynn:Draw()
    ...
    if self.EvelynnMenu.Drawings.DrawQ:Value() then Draw.Circle(myHero.pos, EvelynnQ.range, 1, Draw.Color(255, 0, 191, 255)) end
    if self.EvelynnMenu.Drawings.DrawW:Value() then Draw.Circle(myHero.pos, GetWRange(), 1, Draw.Color(255, 65, 105, 225)) end
    if self.EvelynnMenu.Drawings.DrawE:Value() then Draw.Circle(myHero.pos, EvelynnE.range, 1, Draw.Color(255, 30, 144, 255)) end
    if self.EvelynnMenu.Drawings.DrawR:Value() then Draw.Circle(myHero.pos, EvelynnR.range, 1, Draw.Color(255, 0, 0, 255)) end
    
    for i, enemy in pairs(GetEnemyHeroes()) do
        if self.EvelynnMenu.Drawings.DrawJng:Value() then
        ...
        if self.EvelynnMenu.Drawings.DrawAA:Value() then
            if ValidTarget(enemy) then
